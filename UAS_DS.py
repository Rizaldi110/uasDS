...
if selected == "Home":
    st.title("Selamat Datang di Aplikasi Forecasting - ARIMA")
    st.write("Aplikasi web untuk melakukan forecasting menggunakan metode ARIMA. Aplikasi dapat input data berupa file CSV, fitting model ARIMA, serta menampilkan prediksi dan perbandingan data aktual dengan hasil forecasting. \n\nNB: Pastikan kolom tanggal memiliki format yyyy-mm. ex: 2022-12")
...
if selected == "Plot Data Awal":
    ...
    uploaded_file = st.file_uploader(
        "Upload file CSV Anda\n\nPastikan format kolom date yyyy-mm (2022-12)", type=["csv"])
    ...
    if st.session_state.uploaded_data is not None:
        ...
        data[selected_date_column] = pd.to_datetime(
            data[selected_date_column], format='%Y-%m', errors='coerce')
...
if selected == "Plot ACF & PACF":
    ...
    if len(st.session_state.data_sources) == 0:
        ...
        selected_data_key = st.selectbox(
            "Pilih data untuk plotting:", pilihan)
        selected_data = st.session_state.data_sources[selected_data_key]
        selected_column = st.selectbox(
            "Pilih kolom data:", selected_data.columns)
    ...
    if "uploaded_data" not in st.session_state or st.session_state.uploaded_data is None:
        ...
        if st.button("Forecast"):
            data_aktual[selected_tcol] = pd.to_datetime(
                data_aktual[selected_tcol], format="%Y-%m", errors="coerce")
            ...
            if "sarima_results" not in st.session_state or st.session_state.sarima_results is None:
                st.warning(
                    "Model ARIMA belum di-fit. Silakan fit model di tab sebelumnya!")
            else:
                # in-sample predict
